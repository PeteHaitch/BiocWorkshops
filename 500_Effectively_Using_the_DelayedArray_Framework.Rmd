...
The table below gives some examples of contemporary experiments and the memory requirements if these data are to be stored in-memory as ordinary R arrays.

```{r, echo = FALSE}
x <- data.frame(
    Assay = c("scRNA-seq", "WGBS (mCG)", "WGBS (mCA)"),
    Size = c(
        as.character(
        ...
        as.character(
            2 * memuse::howbig(194756235, 145, type = "integer", prefix = "SI") + 
                memuse::howbig(194756235, 145, type = "double", prefix = "SI"))),
    "`nrow`" = c("27,998", "29,307,073", "194,756,235"),
    "`ncol`" = c("1,306,127", "145", "145"),
    "Number of assays" = c(1, 3, 3),
    Type = c("sparse integer", "2 x dense integer, 1 x dense double",
             "2 x dense integer, 1 x dense double"),
    Reference = c(
...
knitr::kable(x[c(2, 1, 3), ], row.names = FALSE)
```
...
- Learn about the various pieces that make up the DelayedArray framework and how they fit together
...
# NOTE: This comes from http://lazappi.id.au/2018/06/exploring-the-sce-verse/
# NOTE: Using pkg::function() to avoid some NAMESPACE clashes.
...
bpi <- BiocPkgTools::getBiocPkgList()
...
The first kind of package adds support for a new _realization backend_. 
...
Some packages use the DelayedArray framework, including the aforementioned packages, to support the analysis of ever-larger 'omics datasets.
Examples of these include:

- The **bsseq** package used for the analysis of whole-genome bisulfite methylation sequencing experiments.
- The **minfi** package used for the analysis of Illumina methylation array experiments.
- The **scran** and **scater** packages used for the analysis of single-cell gene expression data.
...
* What types of operations 'degrade' an instance of a *DelayedArray* subclass to a *DelayedArray*, as well as when and why this matters.
...
From a developer's perspective, there's a third category of seed that is defined by the *DelayedOp* class. 
Instances of the *DelayedOp* class are not intended to be manipulated directly by the end user, but they are central to the concept of delayed operations, which we will learn about later.
...
The **DelayedArray** package includes routines to compute common summaries of a *DelayedMatrix*, such as base R functions `colSums()` and `rowMeans()`.
Under the hood, these methods are implemented using the block-processing machinery of **DelayedArray**.
If you need to apply a user-defined function over a *DelayedArray*, it may sufficient to apply the function using `DelayedArray::blockApply()` or `DelayedArray::blockReduce()`.

For frequently used summary functions, it makes sense to implement these once and share them via R/Bioconductor packages for reuse by other packages.
As an example of this, the **DelayedMatrixStats** package provides a port of the **matrixStats** API to handle *DelayedMatrix* objects; this includes some 74 row and column summary functions.
**DelayedMatrixStats** also supports "seed-aware" methods that are optimised for speed and memory usage.
The idea is to write methods that target *DelayedMatrix* object with particular types of *seed*.
For example, we can more efficiently compute the column sums of an *RleMatrix* by leveraging the run length encoding of the data than by block-processing and converting blocks to ordinary R arrays.

#### For when you must invent the wheel

Finally, you may decide that you need to implement your algorithm in a lower-level programming language, like C++. 
The **beachmat** package provides a consistent C++ class interface for a variety of commonly used matrix types, including *DelayedMatrix* and *HDF5Matrix* objects.
**beachmat** provides 6 main functions that work the same way across a variety of matrix-like objects:

- Extracting data
    - `get_row()`: Get a row of the object as an *Rcpp::Vector::iterator*
    - `get_col()`: Get a column of the object as an *Rcpp::Vector::iterator*
    - `get()`: Get an element of the object as an integer/double/character value
- Writing data
    - `set_row()`
    - `set_col()`
    - `set()`

A nice feature of implementing an algorithm using **beachmat** is that the same code will work whether it is passed a *DelayedMatrix* or an ordinary *matrix*.
Nonetheless, it does require some extra work, so as with any use of lower-level code in an R package, be sure you really need it before you embark upon using it.

## Incoporating DelayedArray into a package

It requires some work to take an existing package and add support for *DelayedArray* objects.
If the data your package processes are always small enough to be stored and operated on in-memory, then leave it alone!

I've spent a fair bit of time (and gone through several iterations) to add support for the DelayedArray framework to the **bsseq** and **minfi** packages. 
What follows are some lessons I've learnt along the way.

### Learning goals

* Learn common design patterns for writing performant code within the DelayedArray framework.
* Evaluate whether an existing function that operates on an ordinary array can be readily adapted to work on a DelayedArray.
* Reason about potential bottlenecks in algorithms operating on DelayedArray objects.

### Learning objectives

* Design an algorithm that imports data into a *DelayedArray* (specifically, an *HDF5Matrix*).
* Take a function that operates on rows or columns of a matrix and apply it to a DelayedMatrix.
* Use block-processing on a *DelayedArray* to compute:
    * A univariate (scalar) summary statistic (e.g., `max()`).
...
The first step in most analyses is to construct an object to store the data.
This object is typically constructed by importing the output of some upstream bioinformatics software.
We'll step through a much simplified example of such an importer function.
If you are interested, the `bsseq::read.bismark()` function is an example of a more fully-featured importer function.

A good import function should construct an object that is immediately ready for the next step in the analysis pipeline.
It should perform some validity checks on the input.
To ensure the returned object is ready for downstream analysis, it may be opinionated; e.g., `read.bismark()` will sort order the chromosomes according to `GenomeInfoDb::sortSeqlevels()`.
Ideally, the import function should also be fast and memory efficient; e.g., `read.bismark()` achieves this by parallel processing of samples while using an interprocess lock to prevent multiple workers from simultaneously writing to the HDF5 file.

For our example, we'll assume that the file format has columns `chr`, `strand`, `start`, `end`, `count`.
Each file will contain data for a single sample and each file has the same set of loci in the same order.
Our importer function (`import()`) will parse some `files` to contruct a *SummarizedExperiment* where the assay data are stored in an *HDF5Matrix*.


```{r, eval = FALSE}
# NOTE: Returns the HDF5-backed SummarizedExperiment object and saves it as a 
#       side effect.
import <- function(
    # The files to be parsed.
    files, 
    # The BiocParallelParam instance.
    BPPARAM = bpparam(), 
    # The directory where to save the HDF5-backed SummarizedExperiment object.
    dir = tempfile(), 
    # If directory dir already exists, should it be replaced with a new one?
    replace = FALSE, 
    # The dimensions of the chunks and the compression level in HDF5 file.
    chunkdim = NULL, 
    level = NULL) {
    
    # Check compatability of BPPARAM (see 
    # https://github.com/Bioconductor/BiocParallel/issues/76 for details).
    if (!.isSingleMachineBackend(BPPARAM)) {
            stop("The parallelisation strategy must use a single machine ",
                 "when using the 'HDF5Array' realization backend.")
    }
    
    # Setup directory structure to mimic that used by 
    # HDF5Array::saveHDF5SummarizedExperiment().
    HDF5Array:::.create_dir(dir = dir, replace = replace)
    
    # Conctruct the rowRanges.
    ...
    # Construct an interprocess lock (used to prevent parallel writes to the 
    # HDF5 file).
    sink_lock <- BiocParallel::ipcid()
    on.exit(BiocParallel::ipcremove(sink_lock), add = TRUE)
    
    # Construct an ArrayGrid over the columns of the ouput.
    ...
    # Process files in parallel to extract counts and construct assay.
    counts <- bplapply(
        X = seq_along(grid),
    # NOTE: This function writes to 'sink' as a side-effect.
        FUN = function(b, files, grid, sink, sink_lock) {
            file <- files[b]
            x <- read.table(file)
            counts <- x$counts
            viewport <- grid[[b]]
            # NOTE: ipclock() blocks until the lock is obtained.
            ipclock(sink_lock)
            write_block(x = sink, viewport = viewport, block = counts)
            # NOTE: Releases the lock.
            ipcunlock(sink_lock)
        },
        files = files,
        grid = grid,
        ...
        sink_lock = sink_lock,
        BPPARAM = BPPARAM)
    # NOTE: This constructs the HDF5Matrix from 'sink'; it's instantaneous. 
    assay <- as(sink, "DelayedArray")
    
    # Construct SummarizedExperiment.
    se <- SummarizedExperiment(assays = assay, rowRanges = rowRanges)
    
    # Mimic HDF5Array::loadHDF5SummarizedExperiment() to save object.
    x <- se
...
- Discuss fussing about with `BPPARAM` is because `BiocParallel::ipc()` requires that single-machine backend is used.
- Discussing idea behind mimicing `HDF5Array::saveHDF5SummarizedExperiment()`.
...
- Construct a *DelayedArray* from scratch by writing data to a *RealizationSink*.
- Check difference between learning goals and learning objectives
